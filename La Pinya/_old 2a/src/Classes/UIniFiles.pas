...
type
  ...
  public
    class function GetIntValue(const Section, key: string): Integer; static;
...
class function TFileIni.GetIntValue(const Section, key: string): Integer;
var
  Ini: TIniFile;
begin
  Result := -1;
  ...
  Ini := TIniFile.Create(NameIni);
  try
    Result := Ini.ReadInteger(Section, key, -1);
  finally
    FreeAndNil(Ini);
  end;
end;
